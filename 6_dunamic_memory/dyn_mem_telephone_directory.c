...
int main(void)
{
	struct user_information *user = NULL;
	...
	do{
		...
		if(1 == item){
			if (counter !=10){
				if (0 == counter){
					user = malloc(sizeof(struct user_information));
					if(0 == user) {
					...
					} while (strlen(user->first_name) > 12);
					do {
						printf("Enter second name (from 1 to 12 characters):");
						scanf("%s", user->second_name);
					} while (strlen(user->second_name) > 12);
					do {
						printf("Enter number (from 1 to 12 characters):");
						scanf("%s",user->number);
					} while (strlen(user->number) > 12);

				} else {
					user = realloc(user, (counter + 1)*sizeof(struct user_information));
					if(0 == user) {
						printf("MEMORY ALLOCATION ERROR!!!\n");
						exit(EXIT_FAILURE);
					}
				...
				for(i = 0; i < counter; i++) {
					if(!strcmp(find_or_delete_name, (user + i)->first_name)) {
						...
						if(0 == counter) {
							...
							user = realloc(user, counter*sizeof(struct user_information));
							if(0 == user) {
								printf("MEMORY ALLOCATION ERROR!!!\n");
								exit(EXIT_FAILURE);
							}
